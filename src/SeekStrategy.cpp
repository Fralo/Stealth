...
if(locations.size() != 0) {
    if ((gameObject.position.x/ game.map->getTileSize().x)== (locations.at(currentTarget).x / game.map->getTileSize().x) &&
        (gameObject.position.y/game.map->getTileSize().y) == (locations.at(currentTarget).y / game.map->getTileSize().y))
    {

        std::cout<<"Arrived To {x = "<<gameObject.position.x / game.map->getTileSize().x<< "} y = { "<<gameObject.position.y/ game.map->getTileSize().x<<std::endl;
        currentTarget++;
        currentTarget = currentTarget % locations.size();
        std::cout<<"Go To {x = "<<locations.at(currentTarget).x << "} y = { "<<locations.at(currentTarget).y<<std::endl;
    }
