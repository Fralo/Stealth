...
TEST_CASE("Check Line Intersection", "[MathHelper]") {
    ...
    BENCHMARK("MathHelper::checkLineIntersection()") {
                                                         return MathHelper::checkLineIntersection(1, 1, 5, 1, 3, -1, 3, 5);
                                                     };
}
...
TEST_CASE("Check if a target is inside a polygon", "[MathHelper]") {
    ...
    coordinates.emplace_back(1, 1);
    coordinates.emplace_back(1, 5);
    coordinates.emplace_back(5, 1);
...
TEST_CASE("Absolute Coordinates", "[MathHelper]") {
    ...
    REQUIRE((MathHelper::getAbsoluteCoordinates(relatives, pos).x == 2 &&
             MathHelper::getAbsoluteCoordinates(relatives, pos).y == 2));
